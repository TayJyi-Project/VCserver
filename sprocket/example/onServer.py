...
from src.yml import PairYML, SpeakerYML
#argv = {SR, TG, SR.yml, SR-TG.yml, SR_eval.list, data/wav, pair/SR-TG}

def main(*argv):
    argv = argv if argv else sys.argv[1:]
    #relative to app.py
    SourceYml = '../sprocket/example/conf/speaker/' + argv[0] + '.yml'
    PairYml = '../sprocket/example/conf/pair/' + argv[0] + '-' + argv[1] + '.yml'
    OutputFolder = '../sprocket/example/data/pair/' + argv[0] + '-' + argv[1]
    toConvert = '../storage/' + argv[0] + '/tobeconvert.wav'

    argv.extend([toConvert, SourceYml, PairYml, OutputFolder])
    ...
    parser.add_argument('tar', type=str,
                        help='Target speaker')
    parser.add_argument('toConvert', type=str,
                        help='choose a wav to convert')
    ...
    #會變成list, name 是上面定義的，value是相對映的argv
    args = parser.parse_args(argv)

    print(args.toConvert)
...
#########################################################################
    ...
    CHANNELS = 1
    RATE = 44100
    CHUNK_SIZE = 1024    #number of data point to read at a time
    fs, x = wavfile.read(args.toConvert)

    print("Converting")

    x = x.astype(np.float)
    x = low_cut_filter(x, fs, cutoff=30)
    ...
    #記得要轉型astype(np.int16)
    print('Converted output:')
    wavpath = '../storage/' + args.org  + '/afterconvert.wav'
